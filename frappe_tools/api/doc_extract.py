...
Document-agnostic core. When a per-(system, doctype) adapter is registered it
drives the field/line spec, master-data resolution, and document build; without
one, the generic scalar-field path is used.
"""
...
def build_target_schema(target_doctype):
	"""Generic scalar header schema derived from DocType meta (adapter fallback)."""
...
def line_schema(adapter):
	cfg = adapter.line_config() if adapter else None
	return cfg.get("columns") if cfg else None


def _line_table(target_doctype, adapter):
	cfg = adapter.line_config() if adapter else None
	if cfg and cfg.get("table"):
		return cfg["table"]
	return frappe.db.get_value(
		"Document Rule Book",
		{"target_doctype": target_doctype, "enabled": 1, "line_table": ["!=", ""]},
		"line_table",
	)
...
def build_user_prompt(target_doctype, schema, rule_books, line_cols=None, addendum=None):
	...
	if line_cols:
		parts.append("\nLINE ITEM COLUMNS (extract one object per printed line into a `lines` array):\n" + json.dumps(line_cols, indent=2, ensure_ascii=False))
	...
	if line_cols:
		keys = ", ".join(f'"{c["key"]}": <value-or-null>' for c in line_cols)
		out_lines = '{' + keys + ', "page": 1, "bbox": [ymin, xmin, ymax, xmax]}'
		parts.append('\nOUTPUT FORMAT:\n{\n  "fields": [' + out_fields + '],\n  "lines": [' + out_lines + ']\n}')
	else:
		parts.append('\nOUTPUT FORMAT:\n{\n  "fields": [' + out_fields + ']\n}')
...
def extract_document(target_doctype, images):
	...
	extraction.line_table = _line_table(target_doctype, adapter)
	extraction.insert()
...
def run_extraction(extraction):
	...
	try:
		...
		line_cols = line_schema(adapter) if doc.line_table else None
		rule_books = get_rule_books(doc.target_doctype)
		user_prompt = build_user_prompt(
			doc.target_doctype, schema, rule_books, line_cols,
		...
		doc.set("lines", [])
		if line_cols:
			for row in _result_to_lines(data, line_cols, doc.pages):
...
def _build_row(schema_field, item, page_dims):
	...
	# Link values are resolved separately; don't seed `value` with raw printed text.
	...
	return {
		...
		"bbox_json": json.dumps(_normalize_bbox(item.get("bbox"))) if _normalize_bbox(item.get("bbox")) else None,
		"source_page": cint(item.get("page")) or 1,
		"status": "Pending",
	}


def _result_to_lines(data, line_cols, pages):
...
def get_extraction(extraction):
	...
	lines = [{
		"row_no": cint(l.row_no), "description": l.description, "supplier_code": l.supplier_code, "hsn": l.hsn,
	...
	return {
		...
		"line_table": doc.line_table, "pages": pages, "fields": fields, "lines": lines,
	}
...
def update_extraction_line(extraction, row_no, description=None, uom=None, qty=None, rate=None, amount=None):
	"""Edit a line's scalar values (qty/rate/uom/description) from the review UI."""
	doc = frappe.get_doc("Document Extraction", extraction)
	doc.check_permission("write")
...
def create_document_from_extraction(extraction):
	...
	if adapter:
		...
		docname = adapter.build_document(doc)
	else:
		docname = _generic_build(doc)
...
def _generic_build(doc):
	target = frappe.new_doc(doc.target_doctype)
	applied = 0
	for f in doc.extracted_fields:
		if f.status == "Rejected" or f.value in (None, ""):
			continue
		value = coerce.coerce_value(f.fieldtype, f.value)
		if value in (None, ""):
			continue
		target.set(f.fieldname, value)
		applied += 1
	if not applied:
		frappe.throw(_("There are no approved field values to create the document with."))
	target.insert()
	return target.name
